...
@export var rendevouz_address = "192.168.1.2"
#Set the rendevouz port to the port of your third party server
@export var rendevouz_port = 3141
...
func _process(delta):
	"""print("ports tried: " + str(ports_tried))
	print("peer info: " + str(recieved_peer_info))
	print("peer greet: " + str(recieved_peer_greet))
	print("peer confirm: " + str(recieved_peer_confirm))
	print("peer go: " + str(recieved_peer_go))"""
...
func start_traversal(id, is_player_host, player_name):
	...
	var err = server_udp.bind(int(rendevouz_port), "*")
	if err != OK:
...
func _send_client_to_server():
	...
	var buffer = PackedByteArray()
	buffer.append_array((REGISTER_CLIENT+str(client_name)+":"+session_id).to_utf8_buffer())
	server_udp.close()
	server_udp.set_dest_address(rendevouz_address, rendevouz_port)
	server_udp.put_packet(buffer)
